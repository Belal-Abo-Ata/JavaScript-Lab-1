...
function toSafeInteger(num) {
	return Math.max(Math.min(num, Number.MAX_SAFE_INTEGER), Number.MIN_SAFE_INTEGER);
}
...
function getURLParameters(url) {
	return [...new URLSearchParams(url.split('?')[1] || '')].reduce((obj, pair) => {
...
function normalizeUrlSegments(...segments) {
	return new URL(segments.join('/'), 'http://example.com').href.slice('http://example.com'.length);
}
...
function nest(items, idKey = 'id', parentKey = 'parentId', childrenKey = 'children') {
	const nests = [];
	const roots = [];
...
function nthArg(n) {
	return (...args) => args.slice(n < 0 ? args.length + n : n, n < 0 ? n : n + 1)[0];
}
...
function unzip(arrays, fn = (...args) => args) {
	const maxLength = Math.max(...arrays.map((arr) => arr.length));
	return Array.from({ length: maxLength }, (_, index) => fn(...arrays.map((arr) => arr[index])));
}
...
function uncurry(fn, n = 1) {
	...
	return function (...args) {
		...
		const fnToCall = currentArity ? uncurry(original(args.slice(0, currentArity))) : original;
		return currentArity ? fnToCall(args.slice(currentArity)) : original(...args);
	};
}
...
function differenceBy(a, b, fn) {
	...
	return [...a.filter((x) => !setB.has(fn(x))), ...b.filter((x) => !setA.has(fn(x)))];
}
...
function runInWorker(fn, ...args) {
	const worker = new Worker(URL.createObjectURL(new Blob([`(${fn})(${args.join(',')})`])));
...
function join(arr, separator = ',', endSeparator = separator) {
	return arr.reduce(
		(str, val, index) => str + val + (index === arr.length - 1 ? endSeparator : separator),
	);
}
...
const isEmpty = (val) =>
	val == null ||
	!(Object.getPrototypeOf(val) === Object.prototype || val.constructor === Object) ||
...
const is = (type, val) => ![null, undefined].includes(val) && val.constructor === type;

// 209. Get list of elements in both arrays using comparator
const intersection2 = (a, b, comp) => a.filter((x) => b.findIndex((y) => comp(x, y)) !== -1);

// 210. Get list of elements in both arrays after applying function to each
const intersection3 = (a, b, fn) => {
...
const intersection4 = (a, b) => {
	const sB = new Set(b);
	return [...new Set(a.filter((val) => sB.has(val)))];
};

// 212. Create a n-dimensional array with given value
const nDimensionalArray = (value, n) => Array.from({ length: Math.pow(value, n) }).map(() => value);
...
const getType = (val) =>
	val === undefined ? 'undefined' : val === null ? 'null' : val.constructor.name.toLowerCase();
...
const unCamelCase = (str) => str.replace(/([^][A-Z])/g, ($1) => `-${$1.toLowerCase()}`);
...
const flattenObject = (obj, prefix = '') =>
	Object.entries(obj).reduce((res, [key, val]) => {
		...
		return isObjectLike(val) ? { ...res, ...flattenObject(val, pre) } : { ...res, [pre]: val };
	}, {});
...
function isBottomVisible() {
	return (
		window.scrollY > window.innerHeight - document.documentElement.getBoundingClientRect().height
	);
}
